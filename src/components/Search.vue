<template>
  <el-row draggable="false">
    <search-start v-on:StartSearch="StartSearch($event)" style="margin-bottom: 10px;" />
    <transition-group name="el-zoom-in-center">
      <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="2" v-for="item in showdata" :key="item.id" draggable="true"
        @mousedown.native="move" @dragend.native="up(item.id, item.name, item)" :title="item.name">
        <span @click="
          dialogacgweb = true;
        acgdata = item;
        acgdata.AID = item.id;
        acgdata.Title = item.name
          ">
          <a target="_blank">
            <el-card style="border: none" :body-style="{ padding: '0px', background: type_color(item.id) }">
              <div style="position: relative">
                <img draggable="false" v-bind:src="item.cover" class="image" width="100%" :onerror="defalutLogoUrl"
                ...
                <h1 v-if="isNew(item.id)" id="isNew">New!</h1>
                <span id="NewTitle">{{ item.uptodate }}</span>
              </div>
              <div id="Title">
                <span>{{ item.name }}</span>
              </div>
    ...
    <el-dialog :title="acgdata.name" v-model="dialogacgweb" :modal-append-to-body='false'>
...
export default {
  ...
  methods: {
          ...
          if (type != -1) {
            this.$notify.info({
              ...
              message:
                ...
                // />
                h(Info_box, { info_num: type, info_AID: String(this.AID), info_json: this.josn_data },),
              duration: 1000,
            });
          }
          this.reload_();
    ...
    autoadd() {
      if (
        document.querySelector("#app > div > section > section > main >div")
          .offsetHeight <
