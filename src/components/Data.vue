          ...
          <a target="_blank">
            <el-card style="border: none" :body-style="{ padding: '0px', background: type_color(item.AID) }">
              <div style="position: relative">
                <img height="100" draggable="false" v-bind:src="item.PicSmall" :onerror="defalutLogoUrl" class="image"
                  width="100%" style="border-radius: 3px 3px 0px 0px;aspect-ratio: 150 / 208;" />
                <transition-group v-if="reload" name="el-fade-in-linear">
                  <img src="../assets/heart.svg" v-if="ishow(item.AID)" id="love" :key="1" />
                  <div v-if="isBlack(item.AID)" id="black" :key="2">黑名单</div>
                </transition-group>
    ...
    autoadd() {
      ...
      if (
        ...
        document.querySelector("#app > div > section > section > main ")
          .offsetHeight && (_this.Data_type == "recommend" ? _this.page * 100 < _this.MAXnum : _this.page * 20 < _this.MAXnum)
