      ...
      <el-col>
        <el-timeline>
          <transition-group name="el-zoom-in-top">
            <el-timeline-item v-for="(item, index) in showdata" :timestamp="item.time_format_3" placement="top"
              draggable="false" :key="index" type="primary" :title="item.name">
              <span @click="
                dialogacgweb = true;
              acgdata = item;
              acgdata.AID = item.id;
              ...
              ">
                <a target="_blank">
                  <el-card draggable="false" style="border: none" :body-style="{
                    padding: '0px',
                    background: type_color(item.id),
                  }" v-if="reload" :style="{ background: type_color(item.id) }">
                    <el-row>

                      <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="2" draggable="true" @mousedown.native="move"
                        @dragend.native="up(item.id, item.name, item)">
                      ...
                      <el-col :xs="12" :sm="18" :md="20" :lg="21" :xl="22">
                        <h4 id="Title">{{ item.name }}</h4>
                        <p id="Rjiessao">{{ item.intro_clean }}</p>
                      </el-col>
                    </el-row>
...
export default {
  ...
  methods: {
        ...
        document.ondragend = () => {
          ...
          if (type != -1) {
            ...
            this.$notify.info({
              ...
              //   />
              // ),
              message: h(Info_box, { info_num: type, info_AID: String(this.AID), info_json: this.josn_data },),
              showClose: false,
              duration: 1000,
            });
    ...
    loding() {
      ...
      this.contnum = this.save[this.weeks].length;
      this.saveimg = this.save[this.weeks];
